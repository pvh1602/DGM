...
def save_samples(samples, data_dir):

    for i in range(len(samples)):
        image = samples[i].astype('float32') / 3 * 255
        sample = (torch.FloatTensor(image) / 255).permute(2, 0, 1)
        file_name = '{}.png'.format(i)
        fname = os.path.join(data_dir, file_name)
        if not exists(dirname(fname)):
            os.makedirs(dirname(fname))
        save_image(sample, fname)
...
def data_loader(datapath, dataset, batch_size=64):
    if dataset == 'mnist':
        train_data, test_data = load_pickled_data(os.path.join(datapath, 'mnist.pkl'))
        ...
        train_data, test_data = load_pickled_data(os.path.join(datapath, 'celeb.pkl'))
        print(train_data.shape)
        train_data = np.transpose(train_data, (0, 3, 1, 2))
        test_data = np.transpose(test_data, (0, 3, 1, 2))
        print(train_data[0])
...
def discretized_mix_logistic_loss(x, l):
    ...
    # Pytorch ordering
    x = x.permute(0, 2, 3, 1)   # Batch_size x H x W x Channels
    l = l.permute(0, 2, 3, 1)   # Batch_size x H x W x 10*(n_mixture+1)
    xs = [int(y) for y in x.size()]
    ls = [int(y) for y in l.size()]
   
    # here and below: unpacking the params of the mixture of logistics
    nr_mix = int(ls[-1] / 10) 
    logit_probs = l[:, :, :, :nr_mix]   # nr_mix = n_mixture
